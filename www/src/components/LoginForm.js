...
function buildSecondFactorAvailability(factors) {
  ...
  return {
    ...
    [SECOND_FACTOR_METHOD.BACKUP_CODE]: strategies.includes(
      SECOND_FACTOR_METHOD.BACKUP_CODE,
    ),
    [SECOND_FACTOR_METHOD.EMAIL_CODE]: strategies.includes(
      SECOND_FACTOR_METHOD.EMAIL_CODE,
    ),
  };
}
...
export default function LoginForm({ initialMessage = "", nextPath = "" }) {
  ...
  async function finishLogin() {
    ...
    const { error: finalizeError } = await signIn.finalize({
      navigate: ({ session, decorateUrl }) => {
        const taskDestination = session?.currentTask
          ? taskRoutes[session.currentTask.key]
          : null;
        const fallbackDestination =
          destination === "/setup-mfa" ? "/chat" : destination;
  ...
  async function sendSecondFactorChallenge(method) {
    ...
    if (method === SECOND_FACTOR_METHOD.EMAIL_CODE) {
      ...
      if (response.error) {
        throw new Error(
          getErrorMessage(response.error, "Failed to send email code"),
        );
      }
  ...
  async function moveToSecondFactorState() {
    ...
    const availability = buildSecondFactorAvailability(
    ...
    if (!defaultMethod) {
      setError(
        "No supported second-factor strategy is available for this account.",
      );
      return;
    }
  ...
  async function handleSecondFactorMethodChange(nextMethod) {
    ...
    try {
      ...
      setError(
        methodError.message || "Failed to prepare second-factor verification",
      );
    }
  }
  ...
  async function handlePasswordSubmit(event) {
    ...
    try {
      ...
      if (signIn.status === "needs_client_trust") {
        setError(
          "Additional client trust verification is required. Complete the verification challenge and try again.",
        );
        return;
      }
  ...
  async function handleSecondFactorSubmit(event) {
    ...
    try {
      ...
      if (response?.error) {
        setError(
          getErrorMessage(response.error, "Second-factor verification failed"),
        );
        return;
      }
  ...
  return (
    <section className="w-full px-1 sm:px-0">
      <h1 className="cyber-title mt-3 text-2xl font-bold text-foreground">
        Login
      </h1>
      <p className="cyber-muted mt-1 text-sm">Sign in to continue to chat.</p>

      {verifiedMessage ? (
        <p className="cyber-note cyber-note-success mt-4">{verifiedMessage}</p>
      ) : null}
      {error ? (
        <p className="cyber-note cyber-note-error mt-4">{error}</p>
      ) : null}
      ...
      {stage === "password" ? (
        ...
        <form className="mt-5 space-y-5" onSubmit={handleSecondFactorSubmit}>
          <div className="space-y-2">
            <h2 className="cyber-title text-xl font-semibold text-foreground">
              Second factor verification
            </h2>
            <p className="cyber-muted text-sm">
              Second-factor verification is required.{" "}
              {secondFactorInfo ? <>{secondFactorInfo} </> : null}
              Default is <strong>Authenticator app</strong>. If needed, or{" "}
          ...
          <div className="space-y-3">
            <div
              ...
              style={{
                gridTemplateColumns: `repeat(${Math.max(secondFactorOptions.length, 1)}, minmax(0, 1fr))`,
              }}
            >
              {secondFactorOptions.map((option, index) => (
                ...
                >
                  <span className="mr-2 inline-flex align-middle">
                    {getSecondFactorMethodIcon(option.key)}
                  </span>
                  {option.label}
                </button>
              ))}
